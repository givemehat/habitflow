...
def main():
    ...
    if not os.path.exists(binary_path):
        needs_compile = True
    elif os.path.exists(swift_source) and os.path.getmtime(
        swift_source
    ...
    if needs_compile:
        if not shutil.which("swiftc"):
            print(
                "\033[31mError: 'swiftc' compiler not found. Please make sure Command Line Developer Tools are installed.\033[0m"
            )
